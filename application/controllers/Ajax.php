  		...
  		$password = $this->db->escape_str($this->input->post('password', true));
  			...
  			if ($this->Siswa_model->checkUser($username)) {
  				...
  				if ($this->Siswa_model->loginUser($username, $password)) {
   					...
   					$this->session->set_userdata(['user' => $username, 'login' => true, 'status' => 'siswa']);
  			  	} else {
  			  		$response = array(
  			    		'status'=>'Password Anda Salah!'
  			  		);
  				}	
  			} else {
  			  	$response = array(
  			    	'status'=>'Username Anda Salah atau Sudah Pernah Memilih!'
  			  	);
  			}
			...
			if($this->Siswa_model->cekStatusPilih() > 0 && $this->Siswa_model->pilih($session, $id_calon) == true){
			    $response = array(
			      'status'=>'sukses'
			    );
			    $this->logout();
  		...
  		$password = $this->db->escape_str($this->input->post('password', true));
  			...
  			if ($this->Guru_model->checkUser($username)) {
  				...
  				if ($this->Guru_model->loginUser($username, $password)) {
   					...
   					$this->session->set_userdata(['user' => $username, 'login' => true, 'status' => 'guru']);
  			  	} else {
  			  		$response = array(
  			    		'status'=>'Password Anda Salah!'
  			  		);
			...
			if($this->Guru_model->cekStatusPilih() > 0 && $this->Guru_model->pilih($session, $id_calon) == true){
			    $response = array(
			      'status'=>'sukses'
			    );
			    $this->logout();
